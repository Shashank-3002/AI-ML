C      Tetra Cone
     ...
     1 b(2),jn(2,1801),en(2,2,1801),dn(2),eta(1801),f(2,2),qq(5),ee(4)
       open(1,file='Grad.3Ec(6).out',status='new')
       data xx,h,dx/5.0,0.005,0.005/
       m0  = 9999
       Pr  = 7.0
       Re  = 10.0
       Ri  = 10.0
...
C      TEST FOR CONVERGENCE
       ...
       go to 5
 15    continue
       skfx = 2.0*(L1/L4)*sqrt((em + 1.0)/2.0)*fx0
C      httr = - sqrt((em + 1.0)/2.0)*L5*gx0
       write(*,16)x,fx0
       write(1,16)x,skfx
C       write(1,18)
 16    format(4x,'',f9.3,2x,'',f11.5)
C       if( (abs(x - 0.00)).lt.0.0001)go to 555
C       if( (abs(x - 0.5)).lt.0.0001)go to 555
C       if( (abs(x - 1.5)).lt.0.0001)go to 555
       go to 444
  555  write(1,997)
  997  format(4x,'ETA',7x,'F', 7x, 'G')
       do 17 j=1,n0,2
C       write(*,19)eta(j),w(1,j), w(2,j)
C       write(1,19)eta(j),w(1,j), w(2,j)
C  19   format(2x,'',f9.5,2x,'',f9.5,2x,'',f9.5)
  17   continue
  444  do 45 i=1,n0
       aff(i) = af(i)
       ww1(1,i) = w(1,i)
